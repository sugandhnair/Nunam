...
int start_timer(int timeus, void *callback) 
{
	...
	*(TIMER0_BASEADDRESS + BITMODE) = 0x0003; //Set timer resolution to 32 bit
	...
	/*
	 ...
	 how many ticks to reach 509ms ? 
					= 127250 = 0x1F112 ( hex value)
	So it would take 127250 ticks to reach 509ms 
	*/

	//2^6 / (16) = Resolution  in microseconds = 4


	*(TIMER0_BASEADDRESS + CC_0) = timeus/4; // Set capture reg with value obtained from the calculation = 127250

	*(TIMER0_BASEADDRESS + INTENCLEAR) = 0x0000; //Clear timer int just in case

	*(TIMER0_BASEADDRESS + INTENSET) = 0x0100; // Set inteurrpt for CC0 

	*(TIMER0_BASEADDRESS + INTENSET) = 1 << 16; //Setting the 16th bit to enable cc0 int
...
void *printtext() 
{
	printf("The value of counter is : %d" , counter);  //print the value of the counter variable 
}
